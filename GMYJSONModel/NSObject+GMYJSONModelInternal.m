...
NS_INLINE NSString *gmy_cs_to_ns(const char *str) {
  ...
  for (size_t i = 0; i < cnt; i++) {
    ...
    case 'T': {
      ...
      if (p->_ivarType == GMYPropertyEncodingTypeBOOL ||
          (p->_ivarType >= GMYPropertyEncodingTypeShort &&
           p->_ivarType <= GMYPropertyEncodingTypeDouble)) {
        p->_ivarTypeClazz = NSNumber.class;
      }

      if (alist[i].value[0] == '@') {
        p->_ivarTypeClazz = NSClassFromString(
            [tmp substringWithRange:NSMakeRange(2, tmp.length - 3)]);
      }
...
- (NSArray<GMYJSONModelProperty *> *)gmy_lookupInstancePropertyList {
  ...
  while (![NSStringFromClass(curClazz) isEqualToString:metaClassStr]) {
    ...
    for (size_t i = 0; i < count; i++) {
      [ret addObject:[GMYJSONModelProperty
                         propertyWithObjc_property_t:plist[i]]];
    }
    free(plist);
...
- (void)setGmy_propertys:(NSArray<GMYJSONModelProperty *> *)gmy_propertys {
  objc_setAssociatedObject(self, @selector(gmy_propertys), gmy_propertys,
